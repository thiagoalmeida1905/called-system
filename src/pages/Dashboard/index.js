...
export default function Dashboard () {
    ...
    useEffect ( ()=>{
        async function loadChamados() {
            const q = query(listRef, orderBy('created', 'desc'), limit(5)); // montamos a nossa query

            const querySnapshot = await getDocs(q); // vamos ter acesso ao docs de acordo com a nossa query
    ...
    async function updateState( querySnapshot ) { // manipular  e montar a lista para os estados
        ...
        if (!isCollenctionEmpty){
            let lista = [];
            querySnapshot.forEach( doc => {//percorre todo o chamados existentes
                lista.push({
                    id: doc.id,
